...
if ($level == "2") {
    $total_bought = mysqli_fetch_assoc(mysqli_query($con, "SELECT SUM(quantity) AS total FROM orders WHERE user_id='$id' AND order_placed=1"))['total'] ?? 0;
    $total_cart = mysqli_fetch_assoc(mysqli_query($con, "SELECT SUM(count) AS total FROM cart WHERE user_id='$id'"))['total'] ?? 0;
    $total_wishlist = mysqli_fetch_assoc(mysqli_query($con, "SELECT COUNT(*) AS total FROM wishlist WHERE user_id='$id'"))['total'] ?? 0;
?>
